...
import { formatStrToNoSpecialChars, getCurrentDateFormatted } from '@/utils/functions';
...
const processAndSaveImage = async (buffer, originalName, origin = null, id = null) => {
  const now = new Date();
  ...
  fileModifiedName = id ? (id + '-' + Date.now().toString().slice(-3) + '-' + originalName) : fileModifiedName;
  ...
  const data ={
    path: path.join('/uploads', year.toString(), month, path.basename(filePath)),
...
export { upload, handleFileUpload, runMiddleware, processAndSaveImage, processAndSavePdf };
