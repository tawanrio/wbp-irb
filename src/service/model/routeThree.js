...
const routePartner = async (arrRoute) => {
  ...
  if (arrRoute[0] !== 'mecanicas') {
    ...
    const category = await CategoriesProducts.findOne({
      label: arrRoute[1],
    }).lean()
...
const routeProduct = async (category, arrRoute) => {
  ...
  if (geo) {
    ...
    const geoName = stateNameDb || cityNameDb || null

    if (geoName)
      return await routeGeo(hasPartner, arrRoute, category, countries, geoName)
  }
