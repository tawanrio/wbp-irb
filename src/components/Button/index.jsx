    ...
    <>
      {phone && (
        <Link
          ...
          target="_blank"
          style={{
            width: phone?.layout?.width,
            height: phone?.layout?.height,
            backgroundColor: phone?.layout?.colors.bg,
            border: phone?.layout?.border,
          }}
          className="flex flex-col justify-center rounded-2xl bg-slate-800 px-5 py-3"
        >
          <div className="relative flex items-center">
            ...
            <span
              ...
              className={`ml-2 text-center text-lg uppercase opacity-100 md:text-2xl ${phone?.number && 'group-hover:opacity-0'} duration-500 ${phone?.icon.url && 'ml-2'} `}
            >
              {formatedPhoneNumber}
            </span>
            {phone?.number && (
              <span
                ...
                className={`absolute w-full scale-[.82] text-center text-lg uppercase opacity-0 duration-700 group-hover:opacity-100 md:text-2xl ${phone?.icon.url && 'ml-2'} `}
              >
                {formatedPhoneNumber}
              </span>
            )}
        ...
        >
          <div className="relative flex items-center">
            ...
            <span
              ...
              className={`ml-2 text-center text-lg uppercase opacity-100 md:text-2xl ${whatsapp?.number && 'group-hover:opacity-0'} duration-500 ${whatsapp?.icon.url && 'ml-2'} `}
            >
            ...
            {whatsapp?.number && (
              <span
                ...
                className={`absolute w-full scale-[.82] text-center text-lg uppercase opacity-0 duration-700 group-hover:opacity-100 md:text-2xl ${whatsapp?.icon.url && 'ml-2'} `}
              >
                {formatedWhatsappNumber}
              </span>
            )}
