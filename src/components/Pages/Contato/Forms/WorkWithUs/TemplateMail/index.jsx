...
export default function TemplateMailWorkWithUs({ data }) {
  ...
  return (
    ...
    {data.inputs?.info.fullName && (
      <p style={{ color: '#666', marginBottom: '10px' }}>Nome completo: {data.inputs?.info.fullName}</p>
    )}
    ...
    {data.inputs?.info.curriculum && (
      <p style={{ color: '#666', marginBottom: '10px' }}>Currículo: {data.inputs?.info.curriculum}</p>
    )}
